...
exports.getLeaderBoard = ( async (req, res) => {
    ...
    try {
        const data = await db('user_event_participation')
                            .innerJoin('teams','teams.team_id','=','user_event_participation.team_id')
                            .where('user_event_participation.event_id','=',eid)
                            .orderByRaw('ABS(user_event_participation.start_time-user_event_participation.end_time) DESC')
                            .returning('user_event_participation.team_id','users.name','user_event_participation.start_time','user_event_participation.end_time');
